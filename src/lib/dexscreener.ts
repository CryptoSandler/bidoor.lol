...
/**
 * Canonical token metadata, read from DexScreener rather than typed by whoever
 * is paying.
 *
 * This is what stops a bidder from owning an entry's identity: name, ticker,
 * logo and socials all come from the chain's own market data, so topping up
 * someone else's entry cannot rewrite what it says. It also doubles as the
 * existence check — an address no DEX has ever seen cannot be listed.
 */
 ...
 * pair's base token — query WHYPE and the top pair comes back with USDC as
 ...
 * socials should never block a paid listing.
 */
...
export async function fetchTokenMetadata(
  ...
  try {
    ...
    // Timeouts, network failures and non-200s all land here. We fail the bid
    // rather than create an entry with empty metadata — a nameless row on a
    // paid leaderboard is worse than a rejected payment.
    result = {
      ok: false,
      kind: "unavailable",
      message:
