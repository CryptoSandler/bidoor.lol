 ...
 * blocker: the site loads token images from an external CDN and had no policy
 * bounding what else a page could reach, so any future injection had nothing
 * standing between it and an attacker's server.
 *
...
const CSP = [
  ...
  // Token logos come from DexScreener's CDN and nowhere else — the same
  // allowlist the metadata resolver enforces, restated where the browser can
  // enforce it too.
  "img-src 'self' data: https://cdn.dexscreener.com https://dexscreener.com",

  // The browser talks to us; the Solana RPC and DexScreener are called from the
  // server, so they do not belong here.
  "connect-src 'self'",
