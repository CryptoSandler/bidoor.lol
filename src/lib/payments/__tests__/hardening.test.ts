...
describe("the click destination is frozen at creation", () => {
  it("uses the launch link when one was given", () => {
  ...
  it("falls back to the website only at creation time", () => {
    const outcome = placeBid(
  ...
  it("a top-up cannot repoint the destination, even when DexScreener changed", () => {
    placeBid(
      bidFor(USDC_ADDR, 10, null),
      meta({ links: { website: "https://honest-project.example" } }),
    );
  ...
  it("a top-up cannot repoint it when the entry had a launch link either", () => {
    placeBid(bidFor(USDC_ADDR, 10, "https://pump.fun/coin/original"), meta());
    const after = placeBid(
  ...
  it("an entry created with nowhere to point never adopts a website later", () => {
    ...
    const created = placeBid(bidFor(USDC_ADDR, 10, null), meta({ links: {} }));
  ...
  it("seeded entries get a destination too", () => {
...
describe("verification is rate limited", () => {
   ...
   */
  function attempts(bidId: string, ipHash: string, count: number) {
    for (let i = 0; i < count; i++) recordVerificationAttempt(bidId, ipHash);
    db()
      .prepare(`UPDATE verification_attempts SET attempted_at = ? WHERE bid_id = ?`)
      .run(new Date(Date.now() - 10_000).toISOString(), bidId);
  }

  it("caps attempts against a single bid", () => {
    attempts("bid-a", IP, VERIFY_LIMITS.perBid);

    const decision = checkVerificationLimits("bid-a", IP);
  ...
  it("caps attempts from one caller across different bids", () => {
    // Creating more bids must not buy more RPC calls.
    for (let i = 0; i < VERIFY_LIMITS.perIp; i++) recordVerificationAttempt(`bid-${i}`, IP);
    db()
      .prepare(`UPDATE verification_attempts SET attempted_at = ?`)
      .run(new Date(Date.now() - 10_000).toISOString());

    const decision = checkVerificationLimits("bid-fresh", IP);
    expect(decision.ok).toBe(false);
    if (decision.ok) return;
    expect(decision.reason).toBe("too_many_for_ip");
  });

  it("enforces a minimum gap between attempts on one bid", () => {
    recordVerificationAttempt("bid-b", IP);
    const decision = checkVerificationLimits("bid-b", IP);
    expect(decision.ok).toBe(false);
    if (decision.ok) return;
  ...
  it("does not let one caller's attempts block another", () => {
    attempts("bid-c", IP, VERIFY_LIMITS.perBid);
    expect(checkVerificationLimits("bid-c", IP).ok).toBe(false);
    expect(checkVerificationLimits("bid-d", OTHER_IP).ok).toBe(true);
  });

  it("allows a first attempt on a fresh bid", () => {
    expect(checkVerificationLimits("brand-new", IP).ok).toBe(true);
  });
});
...
describe("one bid can only ever have one payment applied", () => {
  function pending(amountUsd = 100) {
    return createPendingBid(bidFor(BONK, amountUsd, "https://pump.fun/coin/x"));
  }

  it("refuses a second payment on the same bid, with a different signature", () => {
    const bid = pending();
    expect(recordPayment(bid.id, "5".repeat(87), 100_000_000n).ok).toBe(true);

    const second = recordPayment(bid.id, "4".repeat(87), 100_000_000n);
    expect(second.ok).toBe(false);
    if (second.ok) return;
    expect(second.reason).toBe("bid_already_paid");
  });

  it("still distinguishes a reused signature from a double-paid bid", () => {
    const first = pending();
    const second = pending();
    recordPayment(first.id, "5".repeat(87), 100_000_000n);

    const reuse = recordPayment(second.id, "5".repeat(87), 100_000_000n);
    expect(reuse.ok).toBe(false);
    if (reuse.ok) return;
    expect(reuse.reason).toBe("signature_used");
  });

  it("lets two different bids each take their own payment", () => {
    const first = pending();
    const second = pending();
    expect(recordPayment(first.id, "5".repeat(87), 100_000_000n).ok).toBe(true);
    expect(recordPayment(second.id, "4".repeat(87), 100_000_000n).ok).toBe(true);
  });
});
