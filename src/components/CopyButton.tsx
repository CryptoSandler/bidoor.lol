...
export function CopyButton({ value, label }: { value: string; label: string }) {
  ...
  return (
    <button
      ...
      className="shrink-0 rounded-pill border border-line-strong px-2.5 py-0.5 text-2xs font-medium text-muted transition-colors hover:border-text hover:text-text"
    >
      {copied ? "Copied" : "Copy"}
    </button>
  );
