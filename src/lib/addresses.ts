...
function checkSolana(raw: string): AddressCheck {
  ...
  if (!decoded || decoded.length !== 32) {
    return {
      ok: false,
      reason: "A Solana mint decodes to 32 bytes (usually 32–44 characters).",
    };
  }
...
function checkTron(raw: string): AddressCheck {
  if (!raw.startsWith("T") || raw.length !== 34 || !BASE58_RE.test(raw)) {
    return {
      ok: false,
      reason: "A TRON contract address starts with T and is 34 characters.",
    };
  }
  const decoded = base58Decode(raw);
...
function checkTon(raw: string): AddressCheck {
  ...
  if (!TON_FRIENDLY_RE.test(raw)) {
    return {
      ok: false,
      reason:
        "A TON jetton address is 48 characters (usually starting EQ or UQ) or raw 0:<64 hex>.",
    };
  }
  ...
  // entry instead of splitting a token's bids across two rows.
