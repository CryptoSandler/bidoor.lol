...
function entry(id: string, bids: [number, number][]): Entry {
  ...
  return {
    ...
    launchpadHost: "pump.fun", launchpadVerified: true, clickUrl: null, links: {}, bids: events, clicks: 0,
...
describe("decay is designed but not switched on", () => {
  it("ships disabled", async () => {
    expect(DECAY).toBeNull();
  });

  it("scores on lifetime total while disabled", async () => {
...
describe("ties", () => {
  it("gives the higher rank to whoever got there first", async () => {
