...
```bash
EV=~/proyectos/evidencia/outbid-tokens/$(date -u +%Y-%m-%d)-$(git branch --show-current); mkdir -p "$EV"
...
**`outbid-tokens` is 3104** — `npm run dev` and `npm start` carry `-p 3104`, and any
...
command agrees with itself. Measured for this repository on 2026-09-02:
    ...
    SKIPS   2025d46 (.claude/ only)
    BUILDS  17927da (scripts/)
